...
def expand_map(img, robot_width):
    ...
    radius = size // 2  # setting the radius of the circle 
    center = (size / 2 - 0.5, size / 2 - 0.5)  # Center of the array

    # Create a 16x16 array of zeros
    circle_array = np.zeros((size, size))

    # Iterate over each element of the array to find which ones are inside the circle
    for i in range(size):
        for j in range(size):
