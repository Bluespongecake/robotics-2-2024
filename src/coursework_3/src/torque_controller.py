...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # part i
        #external_force = np.array([0.75, -9, 0])            # force to accelerate the end effector toward the wall
        
        # part ii
        
        external_force = np.array([0, 1.0, 0])           # force to accelerate the end effector away from the wall
        
        if woundUp == False and y >= y0 + 0.25: # once the arm is far enough away from initial position, trigger the woundUp flag and start the throw
            woundUp = True
        elif woundUp == True:
            external_force = np.array([0.75, -9, 0])
    ...
    while not rospy.is_shutdown():
        ...
        if direction == 'right':
            ...
            external_force += 20.0 * np.array([- x_dot, - y_dot * 0.2, - z_dot, -1*x_omega, 0*y_omega, 1*z_omega]).reshape((6, 1))      # derivative feedback
