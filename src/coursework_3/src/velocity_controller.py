...
class VelocityController(b_pykdl.baxter_kinematics):
    ...
    def ee_IK_solver(self, joint_values, P_des, quat_des, dt, nullspace_en, vel_elbow):
        ...
        if nullspace_en:
            ...
            nj = joint_values.shape[0]
            I = np.eye(nj)
            Proj = I-np.matmul(J_pinv, J_ee)  # this is the null space projector
            ...
            qd2 = (q_desired - joint_values)/dt   #secondary joint velocities
            qd2 = 0.01 * qd2 # artificially slow the speed down here
            ...
            qd2 = np.matmul(J_pinv_elbow, vel_elbow) # secondary joint velocities given vel_elbow
...
def main(task):
    ...
    xyz_des_pick = [0.75, 0, 0.93]
    xyz_des_circle = [0.75, 0.1, 1.23]
    
    rpy_des = [0, np.pi/2, 0]  # this is used only for groups. For individuals it has no effect
