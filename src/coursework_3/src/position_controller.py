...
def main(task):
    ...
    if task == 'initPose':
        ...
        # ========= ========= TASK B part ii ========= =========
        # 1. picking up the brick
        left_arm.servo_to_pose([0.75, 0.5, -0.05], [0.01, np.pi, 0.01])
        left_arm.gripper_close()
        # 2. placing up the brick
        left_arm.servo_to_pose([0.75, 0.25, 0.1],[0.01, np.pi, 0.01])
        left_arm.servo_to_pose([0.75, 0.01, -0.05],[0.01, np.pi, 0.01])
        left_arm.gripper_open()
        # 3. moving arm away
        left_arm.servo_to_pose([0.75, 0.01, 0.1],[0.01, np.pi, 0.01])
        left_arm.servo_to_pose([0.75, 0.5, 0.1],[0.01, np.pi, 0.01])
        # 4. picking up the brick
        right_arm.servo_to_pose([0.75, 0.01, 0.1], [0.01, np.pi, 0.01])
        right_arm.servo_to_pose([0.75, 0.01, -0.05], [0.01, np.pi, 0.01])
        right_arm.gripper_close()
        # 5. placing the brick
        right_arm.servo_to_pose([0.75, -0.25, 0.1],[0.01, np.pi, 0.01])
        right_arm.servo_to_pose([0.75, -0.5, -0.05],[0.01, np.pi, 0.01])
        right_arm.gripper_open()
        # 6. moving arm away
        right_arm.servo_to_pose([0.75, -0.5, 0.1],[0.01, np.pi, 0.01])
        return
        ...
        # 1. picking up the brick
        left_arm.servo_to_pose([0.75, 0.5, -0.05], [0.01, np.pi, 0.01])
        # 2. picking up the brick
        left_arm.gripper_close()
        # 3. lifting to handover position
        left_arm.servo_to_pose([0.75, 0.1, 0.1],[0.01, np.pi, 0.01])
        left_arm.servo_to_pose([0.75, 0.1, 0.1],[((np.pi)/2), np.pi, 0.01])
        # 4. matching with other arm
        right_arm.servo_to_pose([0.75, -0.1, 0.1],[-((np.pi)/2), np.pi, 0.01])
        right_arm.servo_to_pose([0.75, 0.05, 0.1],[-((np.pi)/2), np.pi, 0.01])
        # 5. handover
        right_arm.gripper_close()
        left_arm.gripper_open()
        # moving the left arm out of the way
        left_arm.servo_to_pose([0.75, 0.2, 0.1],[((np.pi)/2), np.pi, 0.01])
        # 6. placing brick at goal
        right_arm.servo_to_pose([0.75, -0.5, 0.1],[0.01, np.pi, 0.01])
        right_arm.servo_to_pose([0.75, -0.5, 0],[0.01, np.pi, 0.01])
        right_arm.gripper_open()
        return
